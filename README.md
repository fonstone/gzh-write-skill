...
| 人格 | 适合 | 风格特点 |
|------|------|---------|
| `ai-cultivator` | AI技术深度号/工程师博客 | 系统化拆解、类比驱动、从问题到方案的工程师叙事 |
...
2. **素材采集**：自动抓取真实数据/引述/案例，锚定在文章中（不编造）
...
```bash
git clone --depth 1 https://github.com/oaker-io/wewrite.git ~/.opencode/skills/wewrite
cd ~/.opencode/skills/wewrite && pip install -r requirements.txt
```
...
编辑 `style.yaml`，设置公众号名称、领域、写作人格、排版主题等。首次运行时 WeWrite 也会引导你完成设置。
...
├── style.yaml                  # 当前风格（公众号名称、领域、人格、封面风格）
...
Step 2  热点抓取 → 历史去重 + SEO → 选题
  ↓
Step 3  框架选择 → 素材采集（webfetch 真实数据）→ 内容增强（按框架类型匹配策略）
  ↓
