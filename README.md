...
"切换到技术科普模式" → "写一篇关于 MMU 的文章"
  ...
  → 七段式骨架 + 问题驱动写作法（替代知识点堆砌）
  → 开头三段式（痛点场景 + 问题定义 + 阅读钩子）+ 禁句黑名单
  → 行文节奏控制（单段 ≤ 3 行 + 原理/类比交替 + 中途悬念钩子）
...
| 框架生成 | 通用五段式骨架 + 7 套变体框架 + tech 模式 4 套 | `references/frameworks.md` + `tech-frameworks.md` |
...
| 行文节奏控制 | 单段 ≤ 3 屏行 + 原理/类比交替 + 每 3-4 小节中途钩子 | `references/tech-writing-guide.md` |
...
| 工程感终审 | L0-L4 五层质检金字塔，含标题/开头/行文/内容/结尾专项校验 | `references/tech-self-check-pyramid.md` |
| 技术布道者人格 | 专为技术科普设计的写作人格（代码优先、类比驱动、来源可追溯） | `personas/tech-explainer.yaml` |
| 四套 tech 框架 | 原理演进型 / 技术对比型 / 实践指南型 / 源码拆解型 | `references/tech-frameworks.md` |
...
4. **问题驱动校验**：所有知识点的展开必须回答"为什么需要/不用会怎样/解决什么痛点"
5. **具象化规则**：禁止"提升很大""延迟降低"等模糊表述，强制替换为具体场景+数值+测试环境
6. **认知负荷控制**：所有缩写首次标注全称，新概念先一句话定义再展开
...
│   │
│   ├── tech-writing-guide.md   # [tech] 技术科普写作规范（开头层/行文层/内容层/结尾层）
│   ├── tech-frameworks.md      # [tech] 4 套技术科普框架（原理演进/技术对比/实践指南/源码拆解）
│   ├── tech-self-check-pyramid.md  # [tech] L0-L4 五层质检金字塔
...
Step 2  热点抓取 → 选题（HKR 权重调整：K=60%/R=30%/H=10%）
  ↓
Step 3  框架选择（4 套 tech 框架）→ 素材采集 → 内容增强（tech 版策略）
  ↓
