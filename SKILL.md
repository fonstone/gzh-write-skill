...
- 用户说"诊断选题"/"这个选题值不值得写"/"这个选题怎么样" → 对用户给出的选题执行**消解漏斗诊断**，按 topic-diagnosis.md 流程：先跑 `python3 {skill_dir}/scripts/topic_diagnosis.py "{选题}" --json --mode {mode}` 完成脚本层扫描，再按消解漏斗五层 + tech 模式技术四维做 LLM 层深度判断。输出标准诊断报告（verdict + 各层结果）。不直接判断"好不好"，而是判断"成不成立"——消解优先于评分。
  ...
  **第二部分：四层质检**（按 self-check-pyramid.md 标准执行 L1-L4）
  1. **L1 硬性规则**：运行 `python3 {skill_dir}/scripts/l1_hard_rules.py {article_path} --json` 与 `python3 {skill_dir}/scripts/check_prose.py {article_path}`（tech 模式追加 `--tech`），逐项修复命中项
...
```
读取: {skill_dir}/references/topic-diagnosis.md
```

对得分最高的 3 个选题执行消解漏斗诊断。**先跑脚本，再跑 LLM 判断**：

```bash
python3 {skill_dir}/scripts/topic_diagnosis.py "{选题}" --json --mode {mode}
```
...
wechat 模式：
```
读取: {skill_dir}/references/human-prose.md 第 1 节（材料门槛）
```
...
if mode == tech:
  ...
  读取: {skill_dir}/references/tech-prose.md（技术版四支柱：技术材料门槛/说话位置/段落推进/技术交付禁令。与 tech-writing-guide.md 并行：tech-prose 管材料/推进/韵律，tech-writing-guide 管叙事/说服/结构）
否则:
  读取: {skill_dir}/references/writing-guide.md
  读取: {skill_dir}/references/human-prose.md（活人感四支柱：材料门槛/说话位置/局部问题推进/交付禁令。与 writing-guide.md 并行：human-prose 管"像人一样推进"，writing-guide 管"反检测统计与排版纪律"）
```
...
**4.0 说话位置**（wechat 与 tech 双模式，动笔前内部回答，答案不交给用户；tech 版五问见 `{skill_dir}/references/tech-prose.md` 第 2 节）：
...
读取: {skill_dir}/references/tech-writing-persuasion.md（技术写作说服力——Cialdini七原则+RISE段落节奏+注意力引导系统，与叙事架构配合：叙事线决定讲什么，说服力决定为什么继续读）
...
```bash
python3 {skill_dir}/scripts/check_prose.py {article_path}
...
否则:
    读取: {skill_dir}/references/wechat-theme-guide.md
...
```bash
# tech 模式自动追加 --tech 标志
python3 {skill_dir}/toolkit/cli.py preview {markdown} --theme tech-pro --tech -o {output}.html
python3 {skill_dir}/toolkit/cli.py publish {markdown} --theme tech-pro --tech --cover {cover} --title "{title}" --digest "{digest}"
```
...
```bash
# 发布（tech 模式追加 --tech）
python3 {skill_dir}/toolkit/cli.py publish {markdown} --cover {cover} --theme {theme} --tech --title "{title}" --digest "{digest}"

# 降级：本地预览（tech 模式追加 --tech）
python3 {skill_dir}/toolkit/cli.py preview {markdown} --theme {theme} --tech --no-open -o {output}.html
```
