...
humanness_score: 46.0
negative_ratio: 0.02
paragraph_cv: 0.68
sentence_stddev: 24.0
short_paragraphs: 15
source: 歸藏的 AI 工具箱
original_title: 开源一个 PPT Skill｜压进了我 10 年的设计经验
original_url: https://mp.weixin.qq.com/s/GXjjlxpIJ604HVCCZJkUBA
...
与 tool-recommendation-001.md（开源日记的专利工具文）相比，本文有一个显著差异：

**本文的 "个人故事铺垫" 不是来自痛苦，而是来自意外** — "大家问得最多的那句话不是分享内容，而是PPT怎么做的"。这种"无意中被认可"的叙事比"我很痛苦"更有作者权威感，适合作者已有一定知名度的场景。
...
工具分享型模板                         本文实际结构
───────────────────────                ───────────────────────
你的真实困境/焦虑       ←→     (不存在痛苦铺垫——用"被认可"替代)
  ↓                                    ↓
偶然发现这个工具         ←→     "没想到副产品成了主产品。索性就把它开源了"
  ↓                                    ↓
自己的测试过程           ←→     "它长什么样" → "它能做什么"（视觉+功能展示）
  ↓                                    ↓
深入的技术拆解           ←→     "字体三级分工""色彩纪律""网格节奏"（设计理念）
  ↓                                    ↓
使用建议和注意事项       ←→     "先跟AI说清这6件事" + "图片规则" + "同名覆盖"
```

**核心区别**：本文在"功能展示"之后多了一层 **"为什么长成这样"**——这是作者作为十年设计师的深度洞见，把工具推荐从"怎么用"升维到了"为什么这样设计"。这是技术类工具推荐文章可以借鉴的高阶写法。

## 关键技术手法拆解

### 1. "副产品变主产品"的叙事反转（第1-4段）

"大家问得最多的一句话是：'那个 PPT 是什么做的，能不能开源一下？'

没想到副产品成了主产品。"

**可复制的手法**：不直接说"我做了个工具"，而是先说一个具体场景（私享会），再说一个意外发现（大家都在问PPT），最后引出工具。这个"副产品"的定位让工具显得谦虚又有价值。

### 2. 第一印象驱动的视觉描述（"它长什么样"）

不是罗列功能，而是先给读者一个**整体印象**：

"打开 Skill 生成的 PPT，第一眼的感觉大概是：这不太像 AI 做的。"

然后用"封面/正文/翻页/元数据"四组对照快速建立起画面感。

**可复制的手法**：在介绍功能之前，先给出**一句话的总印象判断**，然后用3-5个具体特征让读者在脑中建立画面。这比直接列功能清单更能抓住读者。

### 3. 命名即定位："电子杂志 × 电子墨水"

作者给自己的视觉风格起了个名字，让读者一听就能理解。

**可复制的手法**：给自己的方法/风格/产品起一个**有记忆点的组合命名**（"XX × XX"），并说明灵感来源。这让文章在读者脑中留下一个可传播的概念标签。

### 4. "为什么长成这样"的设计理念层

一般的工具推荐文章止步于"怎么用"，但本文增加了一层"为什么"：
...
**可复制的手法**：在介绍完功能和用法之后，加入一节"为什么这样设计"。这展示了作者的专业深度，也让读者不只拿到一个工具，还带走了一个认知框架。

### 5. "约束即自由"的价值观主张

"用户不允许自定义 hex""约束越严，风格越稳。保护美学，比给用户自由更重要。"

这是一个反直觉的主张（通常大家觉得功能越多越好），这种"有态度的设计决策"让人印象深刻。

**可复制的手法**：在工具推荐中找到一个**反直觉但有理有据的观点**，单独拎出来强调。这会让文章在众多"功能清单"式的推荐中脱颖而出。

### 6. "十年压缩包"的收尾升华

"这个 Skill 就是我这十年审美的一个压缩包。"

最后一节没有停留在功能总结，而是用一个精妙的比喻收束全文。然后用"以前花两天→现在只检查"的对比强化价值。

**可复制的手法**：收尾用一个浓缩的比喻（"XX就是一个压缩包"），然后回到个人视角（"我以前怎样，现在怎样"），把工具价值和个人经历绑定。
...
（结构：具体事件 → 意外发现。不写"我开源了一个PPT工具"，而是写"别人找我，然后副产品成了主产品"。这种开头比直接介绍工具更有叙事张力。）

## 情绪高峰

打开 Skill 生成的 PPT，第一眼的感觉大概是：这不太像 AI 做的。

（结构：用一句"反预期"的判断制造好奇——读者会想"AI做的应该是什么样的？它为什么不像是AI做的？"这一定位把工具的价值标准从"AI能不能做"拉高到了"做得好不好看"。）

## 转折/自纠

其实这份 Skill 真正的价值不在模板本身，而在它定义了一套人 × AI 协作做 PPT 的接口。

（结构：在介绍完外观和功能后，作者主动纠正读者的预期——"你以为它是模板？不，它是接口"。把文章从"又一个模板工具"升维到"人机协作方法论"。）

## 收尾

上面这些规则，没有一条是我发明的。我做了十年设计，UI、交互、AI 特效都干过，这些其实都是行业常识。我只是把它们一条条写进了 SKILL.md 和 checklist.md，让 AI 能替我逐条执行而已。

换句话说，这个 Skill 就是我这十年审美的一个压缩包。

以前做一份像样的 PPT，我得花两天手动调网格、选字号、抠色值。现在把素材丢给 AI，它按照这些规则直接拼出来，我只需要检查一下。

（结构：谦逊铺垫（"没有一条是发明的"）→ 经验年限背书 → 精妙比喻（"压缩包"）→ 今昔对比强化价值。整个收尾把工具定位为"专业经验的载体"，而不是"AI的替代品"。）
