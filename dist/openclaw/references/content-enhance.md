...
**前置质检：HKR 选题过滤**：在正式增强之前，先确认选题质量。一个好选题必须通过 HKR 质检：

| 维度 | 问题 | 评审标准 |
|------|------|---------|
| **H (Happy)** | 足够有趣、有悬念吗？ | 标题和开头能让人好奇想点开吗？ |
| **K (Knowledge)** | 有信息量吗？ | 看完能学到新东西吗？ |
| **R (Resonance)** | 能戳中情绪吗？ | 让人"对对对我也这么想"吗？ |

S 级选题三项兼备。及格选题至少占两项。如果选题明显只占一项或零项 → 回 Step 2 调整选题方向或换切入角度。

**角色代入法（发散评估）**：审视选题时代入不同角色问自己：

- **"一个很忙的普通用户"**：能帮我省时间/解决具体麻烦吗？
- **"一个爱玩的朋友"**：够有趣吗？能让我产生"卧槽还能这样？"的惊叹？
- **"一个焦虑的学习者"**：能让我学到新东西、有新角度吗？

三个角色中至少两个回答"是"，才值得投入写作。
...
**tech 模式增强策略替换**（当 style.yaml mode=tech 时生效）：

| 框架类型 | 增强策略 | 核心目标（tech 版） |
|---------|---------|-------------------|
| 原理演进型 / 源码拆解型 | **角度发现** | 找到认知缺口（"80% 开发者误解了这个概念"） |
| 实践指南型 / 技术对比型 | **密度强化** | 知识密度（关键数据/原理图/对比表） |
| 所有 tech 框架 | **细节锚定** | 代码锚/实验锚/性能锚（可复现的测试条件） |
| 技术对比型 | **真实体感** | 业界案例/开源项目实测/NAS 数据 |
