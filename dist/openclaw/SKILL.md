...
- 用户说"诊断选题"/"这个选题值不值得写"/"这个选题怎么样" → 对用户给出的选题执行**消解漏斗诊断**，按 topic-diagnosis.md 流程：先跑 `python3 {baseDir}/scripts/topic_diagnosis.py "{选题}" --json --mode {mode}` 完成脚本层扫描，再按消解漏斗五层 + tech 模式技术四维做 LLM 层深度判断。输出标准诊断报告（verdict + 各层结果）。不直接判断"好不好"，而是判断"成不成立"——消解优先于评分。
  ...
  **第二部分：四层质检**（按 self-check-pyramid.md 标准执行 L1-L4）
  ...
  5. `python3 {baseDir}/scripts/humanness_score.py {article_path} --json` 作为补充参考
...
- 用户说"切换到技术科普模式"/"写技术文章" → 修改 `{baseDir}/style.yaml` 的 `mode` 为 `tech`，告知用户"已切换到技术科普模式"
- 用户说"切回公众号模式"/"写公众号文章" → 修改 `{baseDir}/style.yaml` 的 `mode` 为 `wechat`，告知用户"已切换回公众号模式"
...
- 存在 → 提取 `name`、`topics`、`tone`、`voice`、`blacklist`、`theme`、`cover_style`、`author`、`content_style`，检查 `mode` 字段：
  - `mode == tech` → 额外加载 `{baseDir}/style-tech.yaml`，覆盖 name/tone/voice/topics/core_principles；`writing_persona` 强制设为 `tech-explainer`；告知用户"当前模式：技术科普（OS/AI/嵌入式）"
  - `mode == wechat` 或其他 → 保持原逻辑；**活人感四支柱启用**（wechat 模式专属：材料门槛 Step 3.3 / 说话位置 Step 4.0 / 局部问题推进 Step 4.4 / 交付禁令 Step 4.5+5.3，规范见 `references/human-prose.md`）；告知用户"当前模式：公众号写作"
...
**2.4 选题价值诊断**（强制执行，在 HKR 质检之前）：

```
读取: {baseDir}/references/topic-diagnosis.md
```

对得分最高的 3 个选题执行消解漏斗诊断。**先跑脚本，再跑 LLM 判断**：

```bash
python3 {baseDir}/scripts/topic_diagnosis.py "{选题}" --json --mode {mode}
```

脚本覆盖：Phase 1 选题分类（纯资讯/情绪蹭热度/复杂选题）、Phase 2 第一层语言陷阱词检测、Phase 2 第五层"老生常谈"饱和模式匹配、Phase 3 tech 模式四维度可脚本化部分（模糊性能断言/模糊技术术语/模糊实践信号/领域适配度）。

脚本输出 `verdict`：
- `DISSOLVED` → 选题在脚本层被消解，直接淘汰，从候选列表中替换下一个高分选题重新诊断
- `NEEDS_JUDGMENT` → 脚本通过，进入 LLM 层完成消解漏斗第二/三/四层（假设错误/逻辑错误/事实前提核查）和技术四维的深度判断

**tech 模式技术四维诊断**（替换泛内容感性指标）：
1. **技术准确性**：结论能否经得起验证？版本/环境/参数是否明确？代码/API 是否真实存在？
2. **知识增量**：是否有官方文档无法获得的信息？是否有"只有做过的人才知道"的细节？是否避免老生常谈？
3. **实践可复现性**：能否给出可运行的代码/命令/步骤？环境是否常见可获取？数据是否可复现？
4. **领域适配度**：难度是否匹配 target_audience？是否聚焦到一个可深挖的点？是否在 style-tech.yaml topics 范围内？

**wechat 模式通用四维诊断**（源自 dbs-diagnosis 问题消解思维，替换纯感性指标）：
1. **选题价值**：选题回答的问题是否读者真正会遇到？是否避免情绪化蹭热度？是否避免老生常谈？是否对应 target_audience 真实画像？
2. **信息增量**：是否有官方文档/同类文章无法获得的信息？是否有"只有做过的人才知道"的细节？是否有可独立截图转发的金句？
3. **读者共鸣**：是否戳中具体痛点？是否有可代入的场景/人物/对话？是否避免"对所有人都重要但谁都不在乎"？
4. **领域适配度**：是否在 style.yaml topics 范围内？难度是否匹配 target_audience？是否匹配 content_style？

**诊断原则**：消解优先于评分。漏斗未通过的选题直接淘汰，不参与最终排序。不因为热度高就放过消解漏斗。

诊断结果：✅ PASS → 进入 HKR 质检；❌ DISSOLVED → 替换选题重新诊断；⚠️ NEEDS_ADJUSTMENT → 告知用户调整建议。

**选题通过 HKR 质检**：对诊断通过的选题做快速 HKR（Happy/Knowledge/Resonance）评审：
...
如果 mode=tech，额外加载 `{baseDir}/references/tech-frameworks.md`，合并到框架选择列表中（新增 5 套：原理演进/技术对比/实践指南/源码拆解/全景架构型）。全景架构型用于多层技术全景主题（如 AI Infra 全栈），叙事逻辑为"定位→扫描→锁定→深挖→验证"。
...
```bash
python3 {baseDir}/scripts/fetch_paper.py --search "{关键词}" -o {baseDir}/output/paper-assets/
```

把候选列表（编号/标题/年份/摘要）呈现给用户确认选哪篇，然后用选中编号正式提取：

```bash
python3 {baseDir}/scripts/fetch_paper.py --search "{关键词}" --pick {编号} -o {baseDir}/output/paper-assets/
```

**通道 A（用户直接提供）**——跳过搜索：

```bash
python3 {baseDir}/scripts/fetch_paper.py "{论文 URL 或 PDF 路径}" -o {baseDir}/output/paper-assets/
```

提取完成后读取 `{baseDir}/output/paper-assets/paper.json` 和 `text/` 章节文本，作为素材采集的主来源：**论文锚点（数据/图表/结论，标注"论文 fig.N / 论文 X.X 节"来源）优先**，再按 3.2 补 web_search 素材。

**降级**：arXiv 不可达或 PDF 解析失败 → 告知用户，回退到 Step 3.2 素材采集。

**3.3 材料清点**（双模式强制；wechat 非虚构长稿 ≥1500 字 / tech 长稿 ≥3000 字必须执行）：
...
```
动笔前内部逐条列出 **≥7 件可核验技术锚**（版本锚/API命令锚/代码锚/实验锚/踩坑锚），每件注明来源（官方文档/源码行/实测数据/用户环境）。技术锚数量与目标篇幅挂钩：3000-4000 字至少 7 件，4000-6500 字至少 10 件，6500+ 字至少 12 件。只写概括性类别不算（"Linux 上"不算，"5.4 还是 6.1"才算）。不足 → 按 3.2 再补搜 2 轮 → 仍不足 → **缩小题目（砍掉一个章节）或压缩为 800-1200 字短文**（`material_gate: compressed`）。宁可短而准，绝不用泛化描述和"根据经验"式断言撑篇幅。有论文素材包时，论文锚点（数据/图表/结论）可计入可核验技术锚，来源标注"论文 fig.N / 论文 X.X 节"。
...
```
if mode == tech:
  读取: {baseDir}/references/tech-writing-guide.md（替换 writing-guide.md，含开头层/行文层/内容层/结尾层完整规范）
...
4. 他对哪一点有明确判断。判断依据什么，又到哪里为止。
5. 读者知道上一段以后，最自然会追问什么。

第 3 项太少 → 回到 Step 3.3 重新清点材料。不要先交一篇空稿，再期待用户往里补活人感。
...
>
> 【反翻案腔模式】{seeds.anti_pivot_seeds 随机 1 个}：判断从正面下，先给判断再给依据，不先立一个读者没有的误解再推翻
>
> 【反假细节模式】{seeds.anti_fake_detail_seeds 随机 1 个}：没有来源的精确时间/神态/天气/摆设/对白都是假细节，不写
...
if mode == tech:
  ...
  读取: {baseDir}/references/headline_templates.md（标题生成 SOP——强制使用三类模板之一，标题必须通过三项质检）
  读取: {baseDir}/references/ending_templates.md（结尾模板库——写作 SOP 最后一步，不可跳过）
  读取: {baseDir}/references/tech-common-pitfalls.md（问题驱动写作法的踩坑库配套）
```
...
- **公式写作（tech）**：公式用 `$...$`（行内）/ `$$...$$`（块级）LaTeX 书写，遵循 mathtext 子集（分式 `\frac{}{}`、上下标、根式 `\sqrt{}`、希腊字母、求和 `\sum_{}^{}`、极限 `\lim`）；矩阵等 mathtext 不支持的先试 `\mathbf{}` 语法，仍失败则用表格或文字描述。渲染规则：行内公式自动缩放到正文行高（height:1.5em 基线对齐），块级公式居中显示（max-width:85%），与正文协调不突兀
...
第一优先：叙事架构
  → 决定"讲什么"和"以什么顺序讲"
  → 任何知识点如果不在叙事线上（不服务于背景/问题/演进/方案四要素），即使有信息增量也砍掉
  → 裁决：当说服力的 RISE 结构与叙事张力冲突时，保留叙事张力，调整 RISE 段落配比

第二优先：问题驱动法
  → 决定"每个知识点怎么展开"（why→what→pain→verify）
  → 每个 P0 知识点必须覆盖问题驱动四象限，不可跳过
  → 裁决：当说服力的 Social Proof/Unity 原则与问题驱动法冲突时（比如先讲案例还是先讲痛点的选择），优先问题驱动法的"痛点先行"顺序

第三优先：说服力原则
  → 决定"为什么读者要继续读"（承诺链/证据交付/认知节奏）
  → 在前两层确定的框架内优化表达方式，不改变内容和顺序
  → 裁决：说服力只在叙事架构和问题驱动法已确定的内容和结构上做修辞优化，不能为追求吸引力而修改叙事顺序或跳过问题驱动环节
```

**冲突自检**：每次遇到约束冲突时，问自己两个问题：(1) 这个改动会破坏叙事线的背景→问题→演进→方案顺序吗？(2) 这个知识点有没有完整回答"为什么需要→不用会怎样→解决什么"？如果(1)或(2)任一违反，说服力原则让步。

**非tech模式**的三层约束按对应模式下的写作指南和叙事结构执行，不适用此规则。
...
保存到 `{baseDir}/output/{date}-{slug}.md`。tech 模式文件名使用 `-tech-` 前缀。

**4.5 快速自检**（写完后立即执行，减少 Step 5 重写概率）：

自检分两层执行：**先跑脚本（可量化检查）→ 再跑 LLM 检查（主观判断）**。脚本失败的项直接修复，不进入 LLM 环节。
...
```bash
python3 {baseDir}/scripts/render_formulas.py {article_path} -o {baseDir}/output/{slug}-formulas/
```
...
| 脚本检查项 | 标准 | 通过条件 | 修复方式 |
|-----------|------|---------|---------|
| 禁用词扫描 | writing-guide.md 2.1 禁用词列表 + 自标深度/宣传腔/机械冗长 | 命中数 = 0 | 自动替换为 l1_hard_rules.py 建议用词 |
| 句长方差 | 最短与最长句相差 ≥ 30 字 | L1 报告中的 `sentence_length_range` 通过 | 连续 3 句相近长度 → 拆句或加短句 |
| 句式重复 | "换句话说"/"哪怕A也B"等非翻案高风险句式（翻案句见下方"动作级禁令"行，0 容忍） | 同一种全文 ≤ 1 次 | 第二次出现替换为不同句式 |
...
**第二层：LLM 检查**（脚本全部通过后执行，仅保留需要主观判断的项）：

**内容层面**：
1. **开头钩子**：前 3 句是否用了开头钩子技法（故事场景/问题戳焦虑/数据现象）？如果是平铺直叙的背景介绍，按对应勾法重写开头
2. **增强贯穿**：增强策略的核心输出是否只出现在一段？如果是，在其他 H2 中补充
3. **金句检查**：全文是否有至少 1 句可独立截图转发的句子？如果没有，在情绪高点处补一句

**叙事与结构层面**：
4. **对立面理解**：讲核心观点前是否先站到了对方角度？是否只是单向输出判断而无同理心铺垫？
5. **假设性例子**：有没有出现"比如有一次..."/"假设你是一个..."这类编造场景？有则删掉或改真实案例/标注编辑锚点
6. **回环呼应（契诃夫之枪）**：前文埋的钩子在后面回扣了吗？如果开头有悬念式意象，结尾是否以变化的形式重现？

**磨 + 最高法则**：
7. **反风格 checklist**：逐段检查：在解释→换场景；在罗列→砍到一个；同一论点重复→删第二个；在宣告深度→删宣告；助手都写得出的句子→改或删
8. **口语检验**：逐段读，会这样跟一个聪明的朋友说吗？不会→改。过不了这关的段落整段重写
9. **意外检验**：写这篇时发现了什么自己之前没想到的？它够显眼吗？如果没有意外发现→回去切得更狠
...
if mode == tech:
  读取: {baseDir}/references/tech-self-check-pyramid.md（替换 self-check-pyramid.md）
否则:
  读取: {baseDir}/references/self-check-pyramid.md
```
...
```bash
python3 {baseDir}/scripts/verify_paper.py {article_path} --assets {baseDir}/output/paper-assets/
```
...
**L3 内容质量深度审查**（人工+AI协作）：
- wechat：**通用四维诊断**（选题价值/信息增量/读者共鸣/领域适配度，详见 self-check-pyramid.md L3-W1~W4）+ 观点支撑/知识输出/文化升维/对立面同理心/类型专项/升番/案例公正性/契诃夫之枪 + 压缩试验（删 1/3 后事实/判断/体验不变 → 注水，保留删短版）+ 结尾检查（L4-5，见 self-check-pyramid.md）
- tech：**技术四维诊断**（替换泛内容感性指标——技术准确性/知识增量/实践可复现性/领域适配度，详见 tech-self-check-pyramid.md L3-T1~T4）+ 类比保真度/复杂度控制/争议处理
...
```
if mode == tech:
    按 SKILL.md Step 7.0 技术类主题映射规则（tech-pro/stripe/github/minimal/linear）
否则:
    读取: {baseDir}/references/wechat-theme-guide.md
    按文章原型 + content_style 选择主题，检查调性匹配
```

**技术类文章**（mode=tech）必须从「科技/极简专业类」主题池中选用，避免花哨样式干扰技术信息传递：

| 优先级 | 主题 | 适用 |
|--------|------|------|
| **首选（默认）** | `tech-pro` | 极简专业风：深海军蓝单色强调、无渐变无装饰。代码块为主角，参数表首列等宽对齐。深度技术内容（OS/编译器/嵌入式/分布式原理与源码拆解）默认用此主题 |
| 次选 | `stripe` | Stripe Docs 风：冷静蓝紫强调、有序步骤感。教程/工具说明/操作指南/产品文档类 |
| 次选 | `github` | GitHub README 风：白底、等宽代码、极简。开源/安装说明/工具介绍类 |
| 备选 | `minimal` | 纯黑白极简。方法论/诊断报告类 |
| 备选 | `linear` | Linear Changelog 深色风。版本更新/changelog/路线图类 |
| **禁用** | `wired` | 视觉过强（黄+青+黑色块），仅用于 AI/科技观点/产品发布等冲击型内容，深度技术原理文章禁用 |
| **禁用** | `tech-modern` | 紫蓝渐变装饰过多，技术信息被花哨样式干扰，不推荐 |
| **禁用** | 其他花哨主题（`bauhaus`/`bold-green`/`bold-navy`/`focus-red`/`elegant-rose`/`warm-editorial`/`sspai`/`bytedance`/`midnight`/`newspaper`/`minimal-gold`/`ink`/`professional-clean`） | 装饰性过强或与极简专业风格冲突，技术文章不使用 |

**wechat 模式**：使用 style.yaml 的 `theme` 字段，原逻辑不变。但按 `references/wechat-theme-guide.md` 的主题选择规则做调性匹配检查——如果用户设的主题与文章原型/content_style 明显冲突（如干货型设了 `elegant-rose`），提示调整。

**tech 模式主题映射**（按 tech-frameworks.md 的 5 套框架）：

| 框架 | 默认主题 | 备选 |
|------|---------|------|
| 原理演进型 / 源码拆解型 / 全景架构型 | `tech-pro` | `stripe` |
| 实践指南型 | `stripe` | `tech-pro` |
| 技术对比型 | `tech-pro` | `github` |

**tech-enhance 排版增强**（tech 模式强制开启，wechat 模式可选）：

```bash
# tech 模式自动追加 --tech 标志
python3 {baseDir}/toolkit/cli.py preview {markdown} --theme tech-pro --tech -o {output}.html
python3 {baseDir}/toolkit/cli.py publish {markdown} --theme tech-pro --tech --cover {cover} --title "{title}" --digest "{digest}"
```

`--tech` 开启以下增强（仅 tech-pro/stripe/github/linear/minimal 主题效果最佳）：
- **代码块高亮**：保留 codehilite Pygments 内联高亮，深色背景 `#0f172a` 浅色文字
- **行内标识符分类着色**：寄存器（`%rax`/`x0`/`flag.Z`）→ 暖色；函数/系统调用（`mmap()`）→ 青色；环境变量/参数（`PROT_READ`/`-O2`）→ 紫色；十六进制/二进制字面量（`0x1000`）→ 主色
- **参数表格样式**：表头深色填充+白字；首列等宽字体+主色+nowrap；次列等宽字体+辅色；斑马纹背景。识别参数表（表头含「参数/Parameter/Flag/字段/类型/默认值」等关键词）自动套用
...
| 正文字数 | **wechat 模式** ≥ 1500 字（低于 1500 字公众号基本无推荐量；`material_gate: compressed` 短稿豁免此条）<br>**tech 模式** ≥ 3000 字（深度技术文章可扩展至 6500-8000 字；段三+段四合计 ≥48% 且含原理纵深四层；`material_gate: compressed` 短稿豁免此条） | 低于下限则警告"文章过短，建议补充内容以保证推荐量"（压缩短稿改为提示"材料不足已压缩，宁短而实"） |
...
Converter 自动处理：CJK 加空格、加粗标点外移、列表转 section、外链转脚注、暗黑模式、容器语法。tech 模式额外启用 tech-enhance（代码块高亮、行内标识符分类着色、参数表格样式）。

```bash
# 发布（tech 模式追加 --tech）
python3 {baseDir}/toolkit/cli.py publish {markdown} --cover {cover} --theme {theme} --tech --title "{title}" --digest "{digest}"

# 降级：本地预览（tech 模式追加 --tech）
python3 {baseDir}/toolkit/cli.py preview {markdown} --theme {theme} --tech --no-open -o {output}.html
```

**tech 模式下 `{theme}` 取值**：按 7.0 主题映射规则，默认 `tech-pro`。

**wechat 模式下 `{theme}` 取值**：使用 style.yaml 的 `theme` 字段，不带 `--tech`。
